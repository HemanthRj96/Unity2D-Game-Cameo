...
namespace FickleFrameGames.Systems
{
    ...
    public class Grid<TGridType>
    {
        ...
        public Grid(Vector3 offset, int height, int width, Vector2 cellSize, bool shouldUseText = true, bool shouldUseRenderer = true)
        {
            ...
            cellSize = new Vector2(Mathf.Max(cellSize.x, 0.1f), Mathf.Max(cellSize.y, 0.1f));
        ...
        /// <summary>
        /// Returns the value from grid at an X, Y
        /// </summary>
        /// <param name="x">Target x</param>
        /// <param name="y">Target y</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// Returns the value from grid at a position
        /// </summary>
        ...
        public void DrawWorldText(Color color = default, Transform parentTransform = null)
        {
            ...
            for (int x = 0; x < Width; ++x)
                for (int y = 0; y < Height; ++y)
                    _textGrid[x, y] = UtilityMethods.CreateWorldText("", _grid[x, y].ToString(), parentTransform, GetWorldPosition(x, y) + new Vector3(CellSize.x / 2, CellSize.y / 2), CellSize, color);
        }
        ...
        public void DrawBackground(Sprite sprite = null, Color color = default, Transform parent = null)
        {
            ...
            for (int x = 0; x < Width; ++x)
                for (int y = 0; y < Height; ++y)
                    _spriteGrid[x, y] = UtilityMethods.CreateRenderer("", parent, sprite, GetWorldPosition(x, y) + new Vector3(CellSize.x / 2, CellSize.y / 2), CellSize, color);
        }
    }
}
